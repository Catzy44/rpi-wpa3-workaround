...
### 1 - intro
echo "$S[@] WPA3 workaround for Raspberry Pi... and other stubborn little critters."
...
if [[ "$ARCH" != "aarch64" && "$ARCH" != "arm64" ]]; then
    echo "$S[!] error: unsupported architecture: $ARCH"
...
if [ -d "$BASE_DIR" ]; then
  echo "$S[!] error: Already installed!"
...
read -p "$S[?] - Is your interface wlan0? [Y/n] (it should be)" ANSWER
...
read -p "$S[?] - Do you want to configure the connection now? [Y/n]" ANSWER
if [[ "$ANSWER" != "n" && "$ANSWER" != "N" ]]; then
    sudo nano "$INSTALLER_CONNECTION_CONF_FILE"
fi

### 7 - we're set, lets go!
echo "$S[0] - running the real installation script..."
